...
# CODUL TĂU VINE MAI JOS:
def task_2(*numere):
    return[num for num in numere if isinstance(num, int)]
# CODUL TĂU VINE MAI SUS:
...
# CODUL TĂU VINE MAI JOS:
def task_4(**kwargs):
    return  ' '.join(f"{variabila} {valoare}" for variabila, valoare in kwargs.items())
# CODUL TĂU VINE MAI SUS:
...
def task_5(*args, **kwargs):
    int_list = sorted([arg for arg in args if isinstance(arg, int)])
    string_list = sorted([key for key,value in kwargs.items() if isinstance(value,str)])
...
def task_7(*args):
    ...
    for arg in args:
        if isinstance(arg, str):
            str_lista.append(arg)
        elif isinstance(arg, int):
            int_lista.append(arg)
    return {'str': str_lista, 'int': int_lista}
...
def task_8(*cuvinte):
    ...
    for cuvant in cuvinte:
        invers = cuvant[::-1]
        if invers == cuvant:
            palindrom.append(cuvant)
        else:
            non_palindrom.append(cuvant)
...
# CODUL TĂU VINE MAI JOS:
def task_9(*multipli , number ):
    mlt = []
    for multiplu in multipli:
            if multiplu % number == 0:
                mlt.append(multiplu)
    return mlt
# CODUL TĂU VINE MAI SUS:
...
# CODUL TĂU VINE MAI JOS:
def numara_caractere(cuvant):
    numar_caractere = {}
    for caracter in cuvant:
        if caracter in numar_caractere:
            numar_caractere[caracter] += 1
        else:
            numar_caractere[caracter] = 1
    return numar_caractere

def sunt_anagrame(cuvant1, cuvant2):
    if len(cuvant1) != len(cuvant2):
        return False
    numar_caractere_cuvant1 = numara_caractere(cuvant1)
    numar_caractere_cuvant2 = numara_caractere(cuvant2)
    return numar_caractere_cuvant1 == numar_caractere_cuvant2

def task_13(cuvant, *args):
    for alt_cuvant in args:
        if not sunt_anagrame(cuvant, alt_cuvant):
            return False
    return True
# CODUL TĂU VINE MAI SUS:
...
def task_15(sub_string , *args):
    ...
    for arg in args:
        if sub_string in arg:
            ...
            doesnt.append(arg)
    return {'contains' : contains, 'not_contains': doesnt}
# CODUL TĂU VINE MAI SUS:
...
def task_17(number, **kwargs):
    ...
    for operation, operand in kwargs.items():
        if operation == 'add':
            for arg in operand:
                result += arg
        elif operation == 'sub':
